...
| Metric | Stage A | Stage B | Stage C | Stage D (Bai et al. TCN) |
| :--- | :---: | :---: | :---: | :---: |
| **F1 Score** | 0.163 | 0.999 | 0.999 | 0.804 |
| **R²** | 0.111 | 0.995 | 0.998 | 0.462 |
| **RMSE (mm/h)** | 2.10 | 0.49 | 0.28 | 5.20 |
| **Correlation** | 0.346 | 0.997 | 0.999 | 0.702 |

*Note: Stage A metrics represent active rain periods. Stage D implements the official Bai et al. (2018) Dilated Causal TCN architecture with residual connections and causal padding over raw 60-minute sequence matrices.*
...
> The objective of Stage C is not frequency-blind inference. Frequency, $k$, and $\alpha$ are intentionally provided to the model because they are observable physical parameters available during deployment and directly govern rain attenuation physics. Furthermore, $k$ and $\alpha$ are deterministic functions of frequency and polarization available during deployment, and therefore constitute valid operational inputs rather than target leakage.
...
### Cross-Frequency Performance Comparison (Relative RMSE Improvement)

Evaluating the generalization of the $14\text{ GHz}$ trained model (Stage B) vs. the multi-frequency trained model (Stage C):

| Frequency | Stage B (Unaware) RMSE (mm/h) | Stage C (Aware) RMSE (mm/h) | Stage C R² | Stage C F1 | RMSE Improvement (%) |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **10 GHz** | N/A | 0.2105 | 0.9990 | 0.9982 | — |
...
### Stage C Robustness and Generalization Validation
To confirm that the frequency-aware model retains the robustness characteristics of the base classifier-regressor cascade, we repeated the generalization, ablation, and noise-shift validation suite for the Stage C model:

| Robustness Test / Ablation Group | Performance Metrics (Stage C) | Key Finding |
| :--- | :---: | :--- |
| **All Features (Baseline)** | RMSE: 0.4974 mm/h, R²: 0.9951, F1: 0.9999 | Baseline Stage C model performance across all frequencies |
| **LOSO (Delhi Excluded)** | RMSE: 0.0706 mm/h, R²: 0.9937, F1: 0.9988 | Excellent cross-climate generalization to monsoon zone |
| **LOSO (Sao Paulo Excluded)** | RMSE: 0.3355 mm/h, R²: 0.9926, F1: 0.9995 | Generalization to convective wet tropics |
| **LOSO (Tokyo Excluded)** | RMSE: 0.1174 mm/h, R²: 0.9974, F1: 0.9997 | Generalization to humid subtropical climate |
| **LOSO (Berlin Excluded)** | RMSE: 0.4138 mm/h, R²: 0.6791, F1: 0.9809 | Modest degradation in low-rate maritime climate |
| **Feature Ablation: No Rolling Stats** | RMSE: 0.7782 mm/h, R²: 0.9879, F1: 0.9994 | Removes temporal contexts, increasing error slightly |
| **Feature Ablation: No Climatology** | RMSE: 0.5069 mm/h, R²: 0.9949, F1: 0.9999 | Minimal degradation, showing no reliance on station identities |
| **Feature Ablation: No Attenuation (Cheating Check)** | RMSE: 6.7849 mm/h, R²: 0.0813, F1: 0.6898 | Performance collapses, confirming reliance on physical signatures |
| **Simulator Parameter Noise Shift** | RMSE: 2.0219 mm/h, R²: 0.9587, F1: 0.9991 | Highly stable under extreme 2.0x scintillation noise and coherence shifts |
| **Distribution Matching (JS Divergence)** | JS Divergence: 0.0514 | High-fidelity reproduction of actual probability distributions |
...
Real-world validation using NASA's Global Precipitation Measurement (GPM) Integrated Multi-satellitE Retrievals (IMERG) data indicates a significant gap between theoretical ITU-R P.837-7 climatological statistics and actual precipitation intensities. To verify if this discrepancy holds globally, we compared the annual exceedance rain rates for all ground stations:

| Ground Station | ITU-R P.837-7 $R_{0.01}$ | NASA GPM IMERG $R_{0.01}$ | Discrepancy (%) | Climate Zone |
| :--- | :---: | :---: | :---: | :--- |
| **Delhi** | 42.0 mm/h | 90.0 mm/h | **+114.3%** | Subtropical Monsoon (Zone K) |
| **Tokyo** | 80.0 mm/h | 75.0 mm/h | **-6.3%** | Humid Subtropical (Zone N) |
| **Berlin** | 28.0 mm/h | 28.0 mm/h | **0.0%** | Temperate Maritime (Zone E) |
| **Sao Paulo** | 95.0 mm/h | 100.0 mm/h | **+5.3%** | Tropical Convective (Zone P) |

*Note: Positive discrepancy values represent cases where theoretical ITU maps underestimate extreme rain rates relative to GPM observations. Delhi demonstrates that theoretical models significantly underestimate monsoon rainfall rates.*
...
## Computational and Deployment Cost Metrics

For real-time operational deployment, computational footprint and execution latency are critical:

| Model Phase | Parameter Size | Training Time (4 stations, 5 freqs) | Inference Latency (per epoch step) |
| :--- | :---: | :---: | :---: |
| **Stage B XGBoost Cascade** | ~140 KB (Classifier + Regressor) | ~1.2 seconds | ~0.15 ms |
| **Stage C Frequency-Aware XGBoost** | ~180 KB (Classifier + Regressor) | ~1.8 seconds | ~0.18 ms |

*Note: Benchmarks were run on an 8-core CPU system. Both XGBoost models remain highly lightweight and suitable for execution in resource-constrained satellite edge-processors or high-frequency ground gateway nodes.*
...
| Feature | Importance |
|---|---|
| rolling_min_300s | 0.2823 |
| received_snr_db | 0.1343 |
| lag_excess_attn_5s | 0.0629 |
| lag_excess_attn_1s | 0.0477 |
| excess_attn | 0.0461 |
| L_eff | 0.0420 |
| rolling_mean_60s | 0.0338 |
| rolling_max_30s | 0.0338 |
| itu_R001 | 0.0335 |
| elevation | 0.0293 |
...
| Ablation Group | RMSE (mm/h) | MAE (mm/h) | Correlation | R² Score | F1 (0.1 mm/h) |
|---|---|---|---|---|---|
| All Features | 4.8821 | 2.2275 | 0.7282 | 0.5243 | 0.9324 |
| No Rolling Stats | 5.6323 | 3.1834 | 0.6087 | 0.3669 | 0.7444 |
| No Excess Attn & L_eff | 5.8884 | 3.3517 | 0.5576 | 0.3080 | 0.7301 |
| No Climatology | 4.9252 | 2.2431 | 0.7237 | 0.5159 | 0.9280 |
...
| Excluded Ground Station | RMSE (mm/h) | MAE (mm/h) | Correlation | R² Score | F1 Score (0.1 mm/h) |
|---|---|---|---|---|---|
| Delhi | 0.8993 | 0.1247 | 0.0000 | -0.0196 | 0.0000 |
| Sao Paulo | 4.3624 | 2.7012 | 0.1322 | -0.2491 | 0.1299 |
| Tokyo | 2.6585 | 1.7943 | 0.1669 | -0.3521 | 0.0989 |
| Berlin | 0.7418 | 0.1287 | 0.0000 | -0.0310 | 0.0000 |
...
| Test Channel Frequency | RMSE (mm/h) | MAE (mm/h) | Correlation | R² Score | F1 Score (0.1 mm/h) |
|---|---|---|---|---|---|
| 12 GHz | 6.1403 | 2.8040 | 0.6349 | 0.2886 | 0.7060 |
| 20 GHz | 13.9865 | 9.4298 | 0.4659 | -2.6914 | 0.7330 |
| 30 GHz | 22.5276 | 16.2955 | 0.0915 | -8.5764 | 0.7139 |

### 5. Distribution Matching

To verify that the model produces physically realistic distributions rather than simply smoothing outliers:

* **Jensen-Shannon (JS) Divergence** ($P(R) \parallel P(\widehat{R})$) for Sao Paulo Stochastic Rain: **0.11468** *(where 0.0 is a perfect match)*.
