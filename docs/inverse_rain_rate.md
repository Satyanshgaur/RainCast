...
To confirm that the frequency-aware model retains the robustness characteristics of the base classifier-regressor cascade, we repeated the generalization and noise-shift validation suite for the Stage C model:

| Robustness Test | Evaluation Objective | Performance Metrics (Stage C) |
| :--- | :--- | :--- |
| **LOSO (Delhi Excluded)** | Cross-climate generalization to unseen location | RMSE: 0.0706 mm/h, R²: 0.9937, F1: 0.9988 |
| **LOSO (Sao Paulo Excluded)** | Cross-climate generalization to unseen location | RMSE: 0.3355 mm/h, R²: 0.9926, F1: 0.9995 |
| **LOSO (Tokyo Excluded)** | Cross-climate generalization to unseen location | RMSE: 0.1174 mm/h, R²: 0.9974, F1: 0.9997 |
| **LOSO (Berlin Excluded)** | Cross-climate generalization to unseen location | RMSE: 0.4138 mm/h, R²: 0.6791, F1: 0.9809 |
| **Simulator Parameter Noise Shift** | Robustness to heavy scintillation (2x) & coherence time shifts | RMSE: 2.0219 mm/h, R²: 0.9587, Corr: 0.9822, F1: 0.9991 |
| **Distribution Matching** | Jensen-Shannon (JS) Divergence on Sao Paulo rain rates | JS Divergence: 0.0514 *(where 0.0 is perfect match)* |
